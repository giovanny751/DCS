...
class Lectura_equipo__model extends CI_Model {
    ...
    function consult_lectura_equipo($post) {
        ...
        $this->db->select('pacientes.nombres');
        $this->db->select('variables.hl7tag');
        $this->db->select('lectura_numerica');
        $this->db->select('lectura_texto');
        $this->db->where('lectura_equipo.ACTIVO', 'S');
        $this->db->join('pacientes','pacientes.id_paciente=lectura_equipo.id_paciente');
